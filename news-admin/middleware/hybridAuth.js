...
export function ensureAuthenticated(req, res, next) {
  ...
  if (hubToken && JWT_SECRET) {
    try {
      console.log(`🔍 [News] Verifying JWT with issuer: admin-hub, audience: ${MODULE_ID}`);
      ...
      console.log(`✅ [News] Authenticated via Hub token (user ${decoded.userId})`);
      return next();
    } catch (error) {
      console.error(`❌ [News] Hub token verification failed:`, error.message);
      console.error(`   Token value: ${hubToken.substring(0, 20)}...`);
      // Continue to local session check
    }
  } else {
    console.log(`⚠️  [News] No hub token or JWT_SECRET missing`);
  }

  // Try 2: Local session (cookie news_session)
...
export function handleLogin(req, res) {
  ...
  if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
    ...
    const redirect = returnTo || req.query.redirect || '/news';
