// press-admin/middleware/hybridAuth.js
...
export function ensureAuthenticated(req, res, next) {
  ...
  if (hubToken && JWT_SECRET) {
    try {
      ...
      console.log(`✅ [Press] Authenticated via Hub token (user ${decoded.userId})`);
      return next();
    } catch (error) {
      console.warn(`⚠️  [Press] Hub token invalid:`, error.message);
      // Continue to local session check
    }
  }

  // Try 2: Local session (cookie press_session)
