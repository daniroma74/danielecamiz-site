// newsletter-service/middleware/hybridAuth.js
...
const MODULE_ID = 'newsletter-admin';
...
export function ensureAuthenticated(req, res, next) {
  ...
  if (hubToken && JWT_SECRET) {
    try {
      ...
      console.log(`✅ [Newsletter] Authenticated via Hub token (user ${decoded.userId})`);
      return next();
    } catch (error) {
      console.warn(`⚠️  [Newsletter] Hub token invalid:`, error.message);
      // Continue to local session check
    }
  }

  // Try 2: Local session (cookie newsletter_session)
