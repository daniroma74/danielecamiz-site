// landing/middleware/jwtAuth.js
// JWT Authentication middleware for Admin Hub integration
// SPECIAL: Only protects admin routes, public landing pages remain accessible

import jwt from 'jsonwebtoken';

const JWT_SECRET = process.env.JWT_SECRET;
const ADMIN_HUB_URL = process.env.ADMIN_HUB_URL || 'http://localhost:3100';
const MODULE_ID = process.env.MODULE_ID || 'events-admin';
...
export function requireAuth(req, res, next) {
  // 🌐 IMPORTANT: Only check auth for admin routes (req.isEventAdmin set by routeByDomain middleware)
  if (!req.isEventAdmin) {
    return next(); // Public landing pages don't require authentication
  }
  ...
  try {
    ...
    console.log(`✅ [Landing Auth] User ${req.user.id} authenticated for admin panel`);
    next();

  } catch (error) {
    console.error(`❌ [Landing Auth] Token verification failed:`, error.message);
...
function handleUnauthorized(req, res, reason) {
  console.warn(`⚠️  [Landing Auth] Unauthorized access to admin panel: ${reason}`);
