...
export function ensureAuthenticated(req, res, next) {
  ...
  if (hubToken && JWT_SECRET) {
    try {
      ...
      console.log(`✅ [Landing] Authenticated via Hub token (user ${decoded.userId})`);
      return next();
    } catch (error) {
      console.warn(`⚠️  [Landing] Hub token invalid:`, error.message);
      // Continue to local session check
    }
  }

  // Try 2: Local session (cookie landing_session)
  const sessionId = req.cookies?.landing_session;
...
export function handleLogin(req, res) {
  ...
  if (username === ADMIN_USERNAME && password === ADMIN_PASSWORD) {
    ...
    res.cookie('landing_session', sessionId, {
      httpOnly: true,
...
export function handleLogout(req, res) {
  ...
  res.clearCookie('landing_session');
