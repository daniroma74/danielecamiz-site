// concerts-admin/middleware/jwtAuth.js
...
const MODULE_ID = process.env.MODULE_ID || 'concerts-admin';
...
export function requireAuth(req, res, next) {
  ...
  try {
    ...
    console.log(`✅ [Concerts Auth] User ${req.user.id} authenticated`);
    next();

  } catch (error) {
    console.error(`❌ [Concerts Auth] Token verification failed:`, error.message);
...
function handleUnauthorized(req, res, reason) {
  console.warn(`⚠️  [Concerts Auth] Unauthorized access: ${reason}`);
